...
Graph::Graph(unsigned long vert, unsigned long edges):
    number_of_vertices(vert),
...
Graph Graph::FromStream(std::istream& in) {
  ...
  graph.edges_matrix.reserve(num_of_edges);
...
bool cmp(std::pair<std::pair<int, int>, int>& first, std::pair<std::pair<int, int>, int>& second) {
  return first.second < second.second;
}

int Graph::MST() {
  int weigh = 0;
  std::sort(edges_matrix.begin(), edges_matrix.end(),cmp);
