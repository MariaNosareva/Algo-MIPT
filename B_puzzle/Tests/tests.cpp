...
TEST(SolvingTest, FirstUnsolvable) {
  ...
  solver.SolveIt();
  ASSERT_EQ(solver.get_solution(), "-1");
}
...
TEST(SolvingTest, Example) {
  ...
  PuzzleSolver solver(3, input);
  solver.SolveIt();
  ASSERT_EQ(solver.get_moves(), 8);
  ASSERT_EQ(solver.get_solution(), "RDRULDDR");
}
...
TEST(SolvingTest, SimpleInput) {
  ...
  PuzzleSolver solver(3, input);
  solver.SolveIt();
  ASSERT_EQ(solver.get_moves(), 22);
  ASSERT_EQ(solver.get_solution(), "RDLDRRULLDRUURDDLLURRD");
}
